...
const doWithdrawRewards = createAsyncThunk('crucibleBox/doWithdrawRewards',
    async (payload: {
		network: string,
		crucible: string,
		currency: string,
    ...
    try {
        ...
        let transactionId = await client.withdrawRewards(ctx.dispatch, crucible,currency,  amount,staking);
        if (!!transactionId) {
            ...
            const event = {
                ...
                status: 'pending',
                transactionType: 'WithdrawStakeReward',
...
export function WithdrawStakeCrucible(){
    ...
    console.log(active_crucible,'crucible2',userCrucible)
            ...
            <>
                <FCard className='crucible-filled-card'>
                    ...
                    <div className='subtxt'>
                        You have {Number(userStake?.rewardOf||'0').toFixed(3)} {userCrucible?.symbol} reward(s) available to withdraw.
                    </div>
                    ...
                    :
                        <ApprovableButton
                            ...
                            text={`${transactionStatus==='waiting' ? 'Processing' : 'Withdraw Rewards'}`}
