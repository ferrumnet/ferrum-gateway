...
export class TokenBridgeContractClinet implements Injectable {
	...
	async getSwapEvents(network: string): Promise<BridgeSwapEvent[]> {
		...
		const firstBlock = process.env.BLOCK_LOOK_BACK ?
			...
			(network === 'MUMBAI_TESTNET' ? 990 : network === 'AVAX_TESTNET' ? 277: network === 'MOON_MOONBASE' ? 127: network === 'MOON_MOONRIVER' ? 110: network === 'AVAX_MAINNET'?832:network==='FTM_TESTNET'?602:network==='HARMONY_TESTNET_0'?192:1000);
