...
export class OneInchClient implements Injectable {
	...
	async quote(
		...
		// TODO: Handle the case for no possible routes
		const res: OneInchQuoteRequest = await new Fetcher().fetch<OneInchQuoteRequest>(
			`${QUOTE_API_BASE.replace('{CHAIN_ID}', chainId)}?fromTokenAddress=${fromToken}` + 
