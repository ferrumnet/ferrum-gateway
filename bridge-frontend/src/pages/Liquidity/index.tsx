...
const addLiquidity = async (
  ...
  success: (v: string, tx: string) => void,
  allowanceRequired: boolean
) => {
  try {
    ...
    ValidationUtils.isTrue(
      ...
      "You do Not have Enough Liquidity Available"
    );
...
const tokenSelectedThunk = createAsyncThunk(
  "liquidity/tokenSelected",
  async (payload: { currency: string }, ctx) => {
      ...
      state.data.state.currencyPairs
        .filter(
          (cp) =>
            ...
            cp.targetCurrency === payload.currency
        )
        .forEach((cp) => {
...
async function updateEvent(
  ...
  try {
    const connect = inject<UnifyreExtensionWeb3Client>(
      UnifyreExtensionKitClient
    );
...
function stateToProps(
  ...
  let currency =
    state.currency || (currentIdx >= 0 ? bridgeCurrencies[currentIdx] : "");
  address =
    (addr.filter(
      (e) =>
        e.currency === currency ||
  ...
  const networkOptions = Object.values(supportedNetworks()).filter(
    (n) =>
      ...
      n.active === true &&
      AllowedNetworks.includes(n.key)
  );
  const liqArr = Object.entries(appState.data.state.bridgeLiquidity);
  const liquidityData =
    (liqArr.length > 0 &&
      liqArr.filter(
        (e: any) => e[0]?.split(":")[1] === currency?.split(":")[1]
      )) ||
    [];
...
export function LiquidityPage() {
  ...
  const userAccounts = useSelector<BridgeAppState, AppAccountState>(
    (state) => state.connection.account
  );
  const pageProps = useSelector<BridgeAppState, liquidityPageProps>((state) =>
    stateToProps(state, userAccounts)
  );
  const assets = useSelector<BridgeAppState, FilteredTokenDetails>(
    (state) => state.data.state.filteredAssets
  );
          ...
          <Result
            ...
            extra={[
              <>
                ...
                <a
                  className="text-vary-color"
                  onClick={() =>
                    window.open(
                      Utils.linkForTransaction(pageProps.network, tx),
                      "_blank"
                    )
                  }
                >
          ...
          <div style={styles.container}>
            <div className="pad-main-body">
                    ...
                    <Dropdown className="assets-dropdown liquidity-dropdown">
                      ...
                      <Dropdown.Menu>
                        {pageProps.networkOptions?.map((asset, index) => (
                          <Dropdown.Item
                            eventKey={index}
                            active={asset.key === pageProps.network}
                            disabled={asset.key === pageProps.network}
                            key={index}
                            onClick={() => {
                              switchNetwork(dispatch, asset.key);
                            }}
                          >
                            <span>
                ...
                <div className="content text-left">
                  <div>
                    <AmountInput
                      ...
                      onChange={(v: any) =>
                        amountChanged(dispatch, v.target.value)
                      }
              ...
              <div className="content notif">
                {!pageProps.network ? "Kindly Reconnect" : undefined}
              </div>
            </div>
            <div className="liqu-details">
              <div className="my-liqu-details">
                ...
                <p>
                  ...
                  <ReloadOutlined
                    onClick={() => handleRefresh(pageProps.symbol)}
                    style={{ width: "20px" }}
                    spin={refreshing}
                  />
                </p>
            ...
            <div className="liqu-details">
              <div style={styles.btnCont}>
                <ChainEventItem
                  ...
                  updater={(e) =>
                    updateEvent(dispatch, e, () =>
                      dispatch(
                        tokenSelectedThunk({ currency: pageProps.currency })
                      )
                    )
                  }
                >
                  <LiquidityActionButton
                    ...
                    totalLiquidty={
                      Number(pageProps.TotalAvailableLiquidity) > 0
                        ? Number(pageProps.TotalAvailableLiquidity) - 1
                        : Number(pageProps.TotalAvailableLiquidity)
                    }
                  />
                </ChainEventItem>
              </div>
              ...
              <div>
                <div className="amount-rec-text text-left">
                  {pageProps.liquidityData.length > 0 &&
                    pageProps.liquidityData.map((e) => (
                      <small className="text-pri d-flex align-items-center">
                        Available Liquidity On {e[0].split(":")[0]} ≈{" "}
