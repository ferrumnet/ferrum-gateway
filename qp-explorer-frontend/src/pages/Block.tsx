...
export function Block(props: {}) {
    ...
    const block = useSelector<QpAppState, QuantumPortalMinedBlock|undefined>(state => state.data.state.selectedBlock?.block);
    const dispatch = useDispatch();
    useEffect(() => {
        if (network && hash && init) {
            const client = inject<QpExplorerClient>(QpExplorerClient);
            client.blockByHash(dispatch, network, hash)
    ...
    return (
        <div className="pair-group-container">
            <Pair itemKey={'Time'} value={moment.unix(block.timestamp).toLocaleString()} />
            <Pair itemKey={'Hash'} value={block.blockHash} />
            <Pair itemKey={'Transactions'} value={`${block.transactionCount} transactions`} linkTo={`/txs?network=${block.networkId}&block=${block.blockHash}`}/>
