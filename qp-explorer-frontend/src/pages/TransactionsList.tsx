...
export function TransactionDetailLine(props: {tx: QuantumPortalRemoteTransactoin}) {
    const {tx} = props;
    return (
        <div className="tx-line-container">
            ...
            <div className="tx-line-cell">
                <Link to={`/tx/${tx.hash}?network=${tx.remoteNetworkId}`} >
            ...
            <div className="tx-line-cell">
                <span>{tx.tokenSymbol} {tx.amountDisplay}</span>
            </div>
            <div className="tx-line-cell">
                <span>{tx.gas || ''}</span>
            </div>
...
export function TransactionsList(props: {}) {
    ...
    const txs = useSelector<QpAppState, QuantumPortalRemoteTransactoin[]|undefined>(state => state.data.state.selectedBlock?.transactions);
    const dispatch = useDispatch();
    useEffect(() => {
        if (network && block && init) {
            const client = inject<QpExplorerClient>(QpExplorerClient);
            client.blockByHash(dispatch, network, block)
