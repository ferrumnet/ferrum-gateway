...
import { NetworkedConfig, StakeType, stakeTypeToInt, StakingContract } from "types";
import { CustomTransactionCallRequest } from "unifyre-extension-sdk";
import { RouterV2Staking__factory, StakeOpen, StakeOpen__factory, StakeTimed__factory } from "./resources/typechain";
...
export class StakingService implements Injectable {
	...
	constructor (
		...
		private contracts: NetworkedConfig<StakingContract>,
	) {}

	__name__() { return 'StakingService'; }

	// TODO: Implement allocation...
	async stakeGetTransaction(userAddress: string, stakeType: StakeType, stakeId: string, currency: string, amount: string)
	...
	async takeRewardsGetTransaction(userAddress: string, network: string, stakeType: StakeType, stakeId: string) {
	...
	async stakeInfo(network: string, sType: StakeType, stakeId: string) {
		return this.cache.getAsync(`STAKE_INFO-${network}-${sType}-${stakeId}`, async () => {
			const stake = await this.stake(network, sType);
			const tokens = await this.stake(network, sType).allowedRewardTokenList(stakeId);
			return (tokens || []).map(t => `${network}:${t.toLowerCase()}`)
		}, TEN_MINUTES_MILLI);
	}

	private async rewardCurrencies(network: string, sType: StakeType, stakeId: string): Promise<string[]> {
		return this.cache.getAsync(`REWARD_CURRENCIES-${network}-${sType}-${stakeId}`, async () => {
			const tokens = await this.stake(network, sType).allowedRewardTokenList(stakeId);
			return (tokens || []).map(t => `${network}:${t.toLowerCase()}`)
		});
	}
